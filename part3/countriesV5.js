...
// requête pour récupérer les données des pays
const COUNTRIES_URL = "http://localhost:8000/part3/gateway_to_restcountries.php"  //"https://restcountries.com/v2/all";
let req = new XMLHttpRequest();
req.responseType = "json";
//reqêtes asynchrone donc on ne sait pas quand elle sera terminée c'est pourquoi on utilise un event listener et on éxécute le code dans la fonction load
req.addEventListener("load", function () {
    // La requête est terminée et la réponse est disponible
    let countries = req.response;
    let allCountries = [];
    const LISTCOUNTRIESCONST = [];
    //affichage le la longueur du tableau
    ...
    const UNTOUCHED = listCountriesCopy[0];
    listCountries = listCountries[0]
    listCountriesCopy = listCountriesCopy[0];        
    ...
    document.querySelector(".submitFiltres").addEventListener("click", function () {
        ...
        listCountries = UNTOUCHED; //fill_db();
        let newListe = [];
